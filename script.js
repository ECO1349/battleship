...
// Console obj
var output = {
	"welcome": " > Bienvenido a BattleShip.  Usa el menu para iniciar.",
	"not": " > Esta opcion no esta disponible.",
	"player1": " > Te gustaria colocar tus barcos o que la computadora lo haga automaticamente?",
	"self": " > Coloca tu barco en el tablero inferior.",
	"overlap": " > No puedes encimar los barcos.  Intenta de nuevo.",
	"start": " > Selecciona una casilla del tablero superior para atacar.  Buena suerte!",
	placed: function(name) { return " > Tu " + name + " sido Colocado."; },
	hit: function(name, type) { return " > " + name + " Barco atacado." },
	miss: function(name) { return " > " + name + " Fallaste!" },
	sunk: function(user, type) { return " > " + user + "'s " + type + " hundido!" },
	lost: function(name) { return " > " + name + " Has perdido tu flota!!  Juego Terminado." },
};
	...
	highlight: function(square) {
		...
		$(square).off("click").on("click", function() {
			if(!($(this).hasClass("used"))) {
				...
				if (false == bool) {
					$(".text").text(output.miss("Tu"));
					$(this).children().addClass("perdiste");
...
function gameSetup(t) {
	$(t).off() && $(".two").off();
	$(".one").addClass("self").removeClass("one").text("Colocar tu mismo");
	$(".multi").addClass("random").removeClass("multi").text("Aleatorio");
	...
	$(".random").off("click").on("click", function() {
		playerFleet = new Fleet("Jugador 1");
...
function setShip(location, ship, orientation, genericFleet, type) {
	if (!(checkOverlap(location, ship.length, orientation, genericFleet))) {
		if (orientation == "horz") {
			...
			$(".text").text(output.placed(genericFleet.ships[genericFleet.currentShip].name + " ha sido"));
			for (var i = location; i < (location + ship.length); i++) {
			...
			if (++genericFleet.currentShip == genericFleet.numOfShips) {
				$(".text").text(output.placed("ships han"));
